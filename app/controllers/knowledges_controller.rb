class KnowledgesController < ApplicationController
  ...
  def index
    respond_to do |format|
        format.html { }
        format.json { render json: KnowledgeDatatable.new(view_context,:isAdministrador => current_user.isAdministrador(session[:system_nome],session[:role]), :systemSelected => session[:system_id])}
      end
  end
  ...
  private
    ...
    def knowledge_params
      params.require(:knowledge).permit(:system_id, :user_id, :titulo, :descricao)
    end
end
